...
1. **Write the addon** at `plugins/$ARGUMENTS.py` following the patterns in existing plugins (single-file, `bl_info` header, `register`/`unregister`, `bl_options = {'REGISTER', 'UNDO'}`).

2. **Create the docs folder** at `plugins/$ARGUMENTS/` by copying the structure from `plugins/_template/`:
...
3. **Update the root README** — add a row to the plugins table:
   ```
   | [Plugin Display Name](plugins/$ARGUMENTS/) | One-line description |
   ```
