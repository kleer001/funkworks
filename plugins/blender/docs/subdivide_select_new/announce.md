...
**Subdivide (Select New)** does what Subdivide does, then leaves only the new vertices and edges selected. Mesh menu, right-click menu, or F3. Same `Number of Cuts` / `Smoothness` parameters as the built-in.
...
It lives in the Mesh menu and the right-click context menu in Edit Mode, and it's searchable via F3. The redo panel exposes the same `Number of Cuts` and `Smoothness` parameters as the standard Subdivide. The vert/edge/face selection mode you were in is preserved.
